...
\begin{enumerate}
\item \url{https://gauchospace.ucsb.edu/courses/course/view.php?id=32033}
\item \url{https://piazza.com/class/keu3ggkmq8p66x}
\begin{itemize}
\item Use \url{} to sync new assignments and labs
\end{itemize}
\end{enumerate}
...
\noindent Professor Franks: \url{afranks@pstat.ucsb.edu} OH   Thursday 

\noindent TBD \url{}: OH 4-6pm Thursday 

\noindent TBD \url{}: OH 3-5pm Friday in 5431 B
...
PSTAT 120 A-B and 126 (or equivalent). Familiarity with R is required.
  ...
  \begin{itemize}
    \item There will be approximately 6 homeworks, due roughly every
     week on Sundays at midnight.\\
      \item You are allowed (and encouraged)   to work with a partner. You need only turn in one homework per pair.  If you need help finding a partner, start with the ``Search for
  Teammates'' tool on Piazza.  Homework solutions must be done in
  RMarkdown and turned in on GauchoSpace.  We have provided a homework
  template that you can use to get started.\\
\item All code must be written to be reproducible in Rmarkdown
\item  All derivations can be done in any format of your choosing (latex, written by hand) but must be legible and \emph{must be converted to a pdf}.
\item  All files must be zipped together and submitted to Gauchospace
...
\item Midterm exam (20\%). \textbf{submitted November 9, 2020} . 
\item 10\% quizzes
  \begin{itemize}
    \item Approximately 5 in quizzes.  You have two attempts.
      dropped.
     \item There is no make-up for missed quizzes.
    \end{itemize}
\item Lab attendance (10\%)
  \begin{itemize}
    \item Lab attendance is required and essential.  Your
    attendance will be noted in each lab.
    \end{itemize}
\item Final exam submitted \textbf{Monday December 16, 2020}  (30\%).  
\end{itemize}

\noindent{\textbf{Homeworks:}

\begin{itemize}
\item  All files will be submitted electronically via GauchoSpace
