...
# Objectives
- Bayesian simple linear regression using stan
...
We code the model in a stan file as follows. Note: in stan, if you don't specifiy the prior, it will use the improper priors for the unbounded parameters.
...
Let's say if we are interested in the predict value when x=5, that is $\hat{y} = \alpha+5\beta$. Then we can get samples of this predict value. The sample mean will give us a good estimate of the posterior mean of this predict value at $x=5$.
...
We can also explore the posterior relationship between $\alpha$ and $\beta$.  What is the relationship betweenn these two parameters a posteriori? Can you explain intuitively why this relationship exists?
